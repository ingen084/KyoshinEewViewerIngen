...
震度とマグニチュードによるフィルタリング条件を設定できます。

- 最大震度
  - 予想最大震度でフィルタリングできます。
  - `以上を含む` が有効の場合、指定した震度以上の緊急地震速報がトリガーの対象となります。
  - `以上を含む` が無効の場合、指定した震度と一致する緊急地震速報のみがトリガーの対象となります。
- マグニチュード
  - 指定したマグニチュード以上の緊急地震速報のみをトリガーの対象とします。
  - **注意**: 仮定震源要素の場合はM1.0となります。
- 条件の組み合わせ
  - `両方の条件を満たす (AND)`: 震度とマグニチュードの両方の条件を満たす場合にトリガーされます。
  - `いずれかの条件を満たす (OR)`: 震度またはマグニチュードのいずれかの条件を満たす場合にトリガーされます。
...
- **音量**: 読み上げ音量を0%〜100%の範囲で設定できます。
- **待機**: 有効にすると複数アクション実行時に読み上げが終わるまで次のアクションの動作をブロックします。
- **順次読み上げ**: 改行でテキストを分割し、一括合成後に順番に読み上げます。合成が追いつかない場合は分割単位で読み上げが途切れる可能性がありますが、キャッシュが活用されやすくなります。
...
`順次読み上げ` を有効にすると、テンプレートの改行（`\n`）でテキストを分割し、各セグメントを順番に読み上げます。

例えば以下のようなテンプレートの場合：

```scriban
緊急地震速報
震源は{{ EpicenterPlaceName }}
マグニチュード{{ Magnitude }}
```
