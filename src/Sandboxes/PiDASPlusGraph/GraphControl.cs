...
public class GraphControl : Avalonia.Controls.Control, ICustomDrawOperation
{
	...
	public void Render(IDrawingContextImpl context)
	{
		...
		try
		{
			...
			canvas.DrawText(graphTop.ToString(), new SKPoint(VerticalHeaderSize * .9f - EdgePaint.MeasureText(graphTop.ToString()), EdgePaint.TextSize), EdgePaint);
			canvas.DrawText(graphBottom.ToString(), new SKPoint(VerticalHeaderSize * .9f - EdgePaint.MeasureText(graphBottom.ToString()), (float)Bounds.Height), EdgePaint);
			// 原点
			...
			if (IsIntensityGrid)
				...
				{
					var h = v / (graphTop - graphBottom) * Bounds.Height;
